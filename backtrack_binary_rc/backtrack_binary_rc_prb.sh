#!/bin/bash
#
gfortran -c backtrack_binary_rc_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling backtrack_binary_rc_prb.f90"
  exit
fi
#
gfortran backtrack_binary_rc_prb.o -L$HOME/lib/$ARCH -lbacktrack_binary_rc
if [ $? -ne 0 ]; then
