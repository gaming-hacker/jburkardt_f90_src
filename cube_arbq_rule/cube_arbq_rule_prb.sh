#!/bin/bash
#
gfortran -c cube_arbq_rule_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling cube_arbq_rule_prb.f90"
  exit
fi
#
gfortran -o cube_arbq_rule_prb cube_arbq_rule_prb.o -L$HOME/lib/$ARCH -lcube_arbq_rule
if [ $? -ne 0 ]; then
