#!/bin/bash
#
gfortran -c cube_felippa_rule_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling cube_felippa_rule_prb.f90"
  exit
fi
#
gfortran cube_felippa_rule_prb.o -L$HOME/lib/$ARCH -lcube_felippa_rule
if [ $? -ne 0 ]; then
