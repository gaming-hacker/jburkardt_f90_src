...
!      integer ( kind = 4 ) IND(NCOLS+MCON)
!      integer ( kind = 4 ) IOPT(17+NI)
!      integer ( kind = 4 ) IW(2*(NCOLS+MCON))
!
...
!
  ...
  integer ( kind = 4 ) idope
  integer ( kind = 4 ) idum
  integer ( kind = 4 ) :: igo = 0
  integer ( kind = 4 ) iiw
  integer ( kind = 4 ) ind(*)
  integer ( kind = 4 ) inrows
  integer ( kind = 4 ) iopt(*)
  integer ( kind = 4 ) ip
  integer ( kind = 4 ) irw
  ...
  integer ( kind = 4 ) iw(*)
  integer ( kind = 4 ) j
  integer ( kind = 4 ) jopt(5)
  integer ( kind = 4 ) jp
  integer ( kind = 4 ) lds
  integer ( kind = 4 ) lenx
  integer ( kind = 4 ) level
  integer ( kind = 4 ) liopt
  integer ( kind = 4 ) liw
  integer ( kind = 4 ) llb
  integer ( kind = 4 ) lliw
  integer ( kind = 4 ) llrw
  ...
  integer ( kind = 4 ) lmdw
  integer ( kind = 4 ) lndw
  integer ( kind = 4 ) locacc
  integer ( kind = 4 ) locdim
  integer ( kind = 4 ) lopt
  integer ( kind = 4 ) lp
  integer ( kind = 4 ) lrw
  integer ( kind = 4 ) mcon
...
!       integer ( kind = 4 ) IND(NCOLS), IOPT(1+NI), IW(2*NCOLS)
!
...
!
  ...
  integer ( kind = 4 ) idope
  integer ( kind = 4 ) idum
  integer ( kind = 4 ), save :: igo = 0
  integer ( kind = 4 ) ind(ncols)
  integer ( kind = 4 ) inrows
  integer ( kind = 4 ) iopt(*)
  integer ( kind = 4 ) ip
  integer ( kind = 4 ) :: iscale
...
!
          ...
          if ( lndw < ncols+1) then
              ...
              call xerrwv('dbols(). the column dimension of w(,)=(i1) ' // &
                'must be >= ncols+1=(i2).',nerr,level,2,lndw,ncols+1,0, &
                rdum,rdum)
              go to 190
          end if
          if ( llb < ncols) then
              nerr = 13
              call xerrwv('dbols(). the dimensions of arrays bl(),bu(), ' // &
...
!*****************************************************************************80
!
!! DBOLSM solves E*X = F in the least squares sense with bounds on some X's.
!
...
!       integer ( kind = 4 ) IND(NCOLS), IOPT(1+NI), IBASIS(NCOLS), IBB(NCOLS)
!
...
!
  ...
  logical found
  integer ( kind = 4 ) i
  integer ( kind = 4 ) ibasis(*)
  integer ( kind = 4 ) ibb(*)
  integer ( kind = 4 ) icase
  integer ( kind = 4 ) idope
  integer ( kind = 4 ) idum
  integer ( kind = 4 ) igopr
  integer ( kind = 4 ) ind(ncols)
  ...
  integer ( kind = 4 ) jbig
  integer ( kind = 4 ) jcol
  integer ( kind = 4 ) jdrop
  integer ( kind = 4 ) jdrop1
  integer ( kind = 4 ) jdrop2
  integer ( kind = 4 ) jp
  integer ( kind = 4 ) lds
  integer ( kind = 4 ) level
  integer ( kind = 4 ) lgopr
  integer ( kind = 4 ) lp
  integer ( kind = 4 ) minput
  integer ( kind = 4 ) mode
  integer ( kind = 4 ) mrows
  integer ( kind = 4 ) mval
  integer ( kind = 4 ) nerr
  integer ( kind = 4 ) nlevel
  integer ( kind = 4 ) nsetb
  real ( kind = 8 ), parameter :: one = 1.0D+00
  real ( kind = 8 ) rdum
  real ( kind = 8 ) rnorm
  real ( kind = 8 ) rw(*)
...
!
  do j = 1,ncols
     if ( ind(j) < 1 .or. ind(j) > 4) then
         nerr = 34
         call xerrwv( &
           'dbolsm(). for j=(i1) the constraint indicator must be1-4.'&
                   ,nerr,level,2,j,ind(j),0,rdum,rdum)
         go to 600
     end if
  end do
...
!
      if ( ip > 0) then
          ...
          if ( itmax<=0) then
              nerr = 28
              call xerrwv(&
                'dbolsm(). the maximum number of iterations=(i1) ' // &
...
!
      ...
      if ( mval < 0 .or. mval > minput .or. wt<= 0.0D+00 ) then
          ...
          nlevel = 0
          call xerrwv( &
            'dbolsm(). the row separator to apply weighting (i1) ' // &
            'must lie between 0 and mrows (i2). ' // &
            'weight (r1) must be positive.', &
...
!
!    Throughput, integer ( kind = 4 ) IOPT(*), parameters to be passed to FUNC.
!
!    Input, integer ( kind = 4 ) LDFJ, the leading dimension of FJ, which must
!    be at least MEQUA+MCON.
!
!    Input, integer ( kind = 4 ) MCON, the number of constraints.
!
!    Input, integer ( kind = 4 ) MEQUA, the number of nonlinear functions.
!
...
!    Workspace, real ( kind = 8 ) FX(MEQUA+MCON).
!
!    Throughput, integer ( kind = 4 ) IOPT(*), parameters to be passed
!    to FUNC.
!
!    Input, integer ( kind = 4 ) LDFJ, the leading dimension of FJ, which must
!    be at least MEQUA+MCON.
...
!
!        integer ( kind = 4 ) MEQUA, NVARS, MCON, LDFJ, IGO
!        integer ( kind = 4 ) IND(NVARS+MCON), IOPT(LIOPT), IWORK(LIWORK)
!
...
!      integer ( kind = 4 ) IND(5),IOPT(24),IWA(084)
!
!      EXTERNAL DQEDEX
!
...
!
  ...
  integer ( kind = 4 ) i
  integer ( kind = 4 ) idum
  integer ( kind = 4 ), save :: iflag = 0
  integer ( kind = 4 ) igo
  integer ( kind = 4 ), save :: iiwaav
  integer ( kind = 4 ) ind(*)
  integer ( kind = 4 ) iopt(*)
  integer ( kind = 4 ) iwa(*)
  integer ( kind = 4 ) iwaav
  integer ( kind = 4 ) j
  integer ( kind = 4 ) jp
  integer ( kind = 4 ) kp
  integer ( kind = 4 ), save :: level
  integer ( kind = 4 ) lp
  integer ( kind = 4 ) lpdiff
  integer ( kind = 4 ) mb
  integer ( kind = 4 ) mbb
  integer ( kind = 4 ) mblb
  integer ( kind = 4 ) mbub
  ...
  integer ( kind = 4 ) mequa
  integer ( kind = 4 ) mgr
  integer ( kind = 4 ), save :: milast
  integer ( kind = 4 ), save :: mind
  integer ( kind = 4 ), save :: mindb
  integer ( kind = 4 ), save :: mpj
  integer ( kind = 4 ), save :: mqc
  integer ( kind = 4 ), save :: mwa
  integer ( kind = 4 ), save :: mwj
  integer ( kind = 4 ), save :: mwlast
  integer ( kind = 4 ), save :: mxb
  integer ( kind = 4 ), save :: mxp
  integer ( kind = 4 ), save :: mzp
  integer ( kind = 4 ), save :: nall
  logical, save :: noquad
  integer ( kind = 4 ), save :: npmax
  integer ( kind = 4 ) nvars
...
!
!    Input, integer ( kind = 4 ) LDFJ, the leading dimension of FJ, which
...
!
!    Input, integer ( kind = 4 ) IOPT(*), the integer ( kind = 4 ) option array.
!
...
! integer ( kind = 4 ), REAL, real, LOGICAL, CHARACTER
...
! IGO     DUMMY-ARG     integer ( kind = 4 )           direct model action
! IND     DUMMY-ARG     integer ( kind = 4 )   ADJ-ARY Model bound indicators
! IOPT    DUMMY-ARG     integer ( kind = 4 )   ADJ-ARY Option array
! IWA     DUMMY-ARG     integer ( kind = 4 )   ADJ-ARY Working array
! LDFJAC  DUMMY-ARG     integer ( kind = 4 )           Row dim of FJAC(*,*)
! MB      /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to B(*)
! MBB     /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to BB(*)
! MBLB    /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to BLB(*)
! MBUB    /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to BLB(*)
! MCON    DUMMY-ARG     integer ( kind = 4 )           Number, model constraints
! MDX     /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to DX(*)
! MEQUA   DUMMY-ARG     integer ( kind = 4 )           Number, model equations
! MINDB   /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to INDB(*)
! MIWA    /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to IWA(*)
! MWA     /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to WA(*)
! MXB     /S$A$V$E/ SAV integer ( kind = 4 )           Pointer to XB(*)
! NALL    /S$A$V$E/ SAV integer ( kind = 4 )           NVARS+MEQUA
! NVARS   DUMMY-ARG     integer ( kind = 4 )           Number, user variables
...
!  integer ( kind = 4 ) WORK ARRAYS, WA(*) AND IWA(*).
!
...
!
  ...
  integer ( kind = 4 ) igo
  integer ( kind = 4 ) igoiov
  integer ( kind = 4 ) igopoa
  integer ( kind = 4 ) igotfc
  integer ( kind = 4 ) igotnc
  integer ( kind = 4 ) ind(*)
  integer ( kind = 4 ) indb(*)
  integer ( kind = 4 ) iopt(*)
  integer ( kind = 4 ) ipls
...
!
  ...
  integer ( kind = 4 ) :: iflag = 0
  integer ( kind = 4 ) igo
  integer ( kind = 4 ) igoelm
  integer ( kind = 4 ) igoeqm
  integer ( kind = 4 ) igotfc
  integer ( kind = 4 ) igow
  integer ( kind = 4 ) ind(*)
  integer ( kind = 4 ) indb(*)
...
!
!    Input, real ( kind = 8 ) DX(N), the vector to be printed.
!
!    Input, character ( len = * ) IFMT, a heading or title.
!
!  IDIGIT  PRINT AT LEAST IABS(IDIGIT) DECIMAL DIGITS PER NUMBER.
!          THE SUBPROGRAM WILL CHOOSE THAT integer ( kind = 4 ) 6,14,20 OR 28
!          WHICH WILL PRINT AT LEAST IABS(IDIGIT) NUMBER OF
...
!*****************************************************************************80
!
!! IVOUT prints integer ( kind = 4 ) vectors.
!
!  Modified:
!
!    28 July 2006
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the size of the vector IX.
!
!    Input, integer ( kind = 4 ) IX(N), the array to be printed.
!
!    Input, character ( len = * ) TITLE, a title to be printed.
!
!    Input, integer ( kind = 4 ) IDIGIT, indicates the number of digits
!    to print.  Print up to iabs(idigit) decimal digits per number.
!    the subprogram will choose that integer ( kind = 4 ) 4,6,10 or 14
!    which will print at least iabs(idigit) number of
!    places.  if idigit.lt.0, 72 printing columns are utilized
...
!  Parameters:
!
!    Input, character ( len = * ) XMESS, the message to be processed.
!
