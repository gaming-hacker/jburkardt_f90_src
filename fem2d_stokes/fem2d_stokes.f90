...
!  Local parameters:
!
!    Local, real ( kind = 8 ) A(3*IB+1,VARIABLE_NUM), the VARIABLE_NUM
!    by VARIABLE_NUM coefficient matrix, stored in a banded format.
!
...
!
!    Local, integer ( kind = 4 ) NODE_P_CONDITION(NODE_NUM),
...
!
!    Local, integer ( kind = 4 ) NODE_P_VARIABLE(NODE_NUM),
!    is the index of the pressure variable associated with the node,
!    or -1 if there is no associated variable.
!
!    Local, integer ( kind = 4 ) NODE_TYPE(NODE_NUM), determines if the node is
!    a vertex or midside node.
...
!
!    Local, integer ( kind = 4 ) NODE_U_CONDITION(NODE_NUM),
...
!    3, a Neumann condition is used.
!
!    Local, integer ( kind = 4 ) NODE_U_VARIABLE(NODE_NUM),
!    is the index of the horizontal velocity variable associated with the node,
!    or -1 if there is no associated variable.
!
!    Local, integer ( kind = 4 ) NODE_V_CONDITION(NODE_NUM),
...
!    3, a Neumann condition is used.
!
!    Local, integer ( kind = 4 ) NODE_V_VARIABLE(NODE_NUM),
...
!
!    Local, integer ( kind = 4 ) NODE3_NUM, the number of order 3 nodes.
!
!    Local, integer ( kind = 4 ) NODE3_LABEL(NODE_NUM), contains the renumbered
!    label of order3 nodes, and -1 for nodes that are not order3 nodes.
!
!    Local, real ( kind = 8 ) NU, the kinematic viscosity.
!
!    Local, integer ( kind = 4 ) QUAD_NUM, the number of quadrature points used
!    for assembly.  This is currently set to 3, the lowest reasonable value. 
!    Legal values are 1, 3, 4, 6, 7, 9, 13, and for some problems, a value of 
!    QUAD_NUM greater than 3 may be appropriate.
!
...
!
  ...
  integer ( kind = 4 ), allocatable, dimension(:) :: node_p_condition
  integer ( kind = 4 ), allocatable, dimension(:) :: node_p_variable
  integer ( kind = 4 ) node_show
  integer ( kind = 4 ), allocatable, dimension(:) :: node_type
  integer ( kind = 4 ), allocatable, dimension(:) :: node_u_condition
  integer ( kind = 4 ), allocatable, dimension(:) :: node_u_variable
  integer ( kind = 4 ), allocatable, dimension(:) :: node_v_condition
  integer ( kind = 4 ), allocatable, dimension(:) :: node_v_variable
  real ( kind = 8 ), allocatable, dimension(:,:) :: node_xy
  integer ( kind = 4 ) node3_num
  integer ( kind = 4 ), allocatable, dimension(:) :: node3_label
  real ( kind = 8 ) :: nu = 1.0D+00
  integer ( kind = 4 ) p_node
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'FEM2D_STOKES'
  write ( *, '(a)' ) '  FORTRAN90 version:'
  write ( *, '(a)' ) '  Finite element solution of the '
  write ( *, '(a)' ) '  steady incompressible Stokes equations'
  write ( *, '(a)' ) '  on a triangulated region in 2 dimensions.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  -nu * ( Uxx + Uyy ) + dPdx = F1(x,y)'
  write ( *, '(a)' ) '  -nu * ( Vxx + Vyy ) + dPdy = F2(x,y)'
...
!
  ...
  if ( element_order /= 6 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'FEM2D_STOKES - Fatal error!'
...
!
  call triangulation_order6_bound_node ( node_num, element_order, &
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '      Node   U_index   V_index   P_index'
  write ( *, '(a)' ) ' '
  do node = 1, node_num
    write ( *, '(2x,i8,2x,i8,2x,i8,2x,i8)' ) &
...
!
  if ( debugging ) then
    ...
    call r8vec_print_some ( variable_num, f, 1, 20, &
...
!
  ...
  if ( .true. ) then
    ...
    call dgb_print_some ( variable_num, variable_num, ib, ib, a, 639, 639, &
      659, 659, &
      '  Part of Stokes matrix, adjusted for Dirichlet BC:' )

    call r8vec_print_some ( variable_num, f, 639, 659, &
      '  Part of Stokes right hand side, adjusted for Dirichlet BC:' )
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Variables associated with nodes:'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '      Node       U               V               P'
  write ( *, '(a)' ) ' '
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Velocities written to "' // trim ( file_name ) // '".'
!
!  Free memory.
!
  deallocate ( a )
...
!    in an element.
!
!    Input, integer ( kind = 4 ) VARIABLE_NUM, the number of unknowns.
!
!    Input, real ( kind = 8 ) NODE_XY(2,NODE_NUM), the coordinates
!    of the nodes.
!
!    Input, integer ( kind = 4 ) NODE_P_VARIABLE(NODE_NUM), the index of the
!    pressure variable associated with a node, or -1 if there is none.
!
!    Input, integer ( kind = 4 ) NODE_U_VARIABLE(NODE_NUM), the index of the
!    horizontal velocity variable associated with a node, or -1 if 
!    there is none.
!
!    Input, integer ( kind = 4 ) NODE_V_VARIABLE(NODE_NUM), the index of the 
!    vertical velocity variable associated with a node, or -1 if there is none.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes that 
!    form each element.  Nodes 1, 2, and 3 are the vertices.  Node 4 is 
!    between 1 and 2, and so on.
!
!    Input, real ( kind = 8 ) NU, the kinematic viscosity.
!
!    Input, integer ( kind = 4 ) IB, the matrix half-bandwidth.
!
...
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 4 ) NODE_P_VARIABLE(NODE_NUM),
!    is the index of the pressure variable associated with the node,
...
!    is the index of the horizontal velocity variable associated with the node.
!
!    Input, integer ( kind = 4 ) NODE_V_VARIABLE(NODE_NUM),
!    is the index of the vertical velocity variable associated with the node.
!
!    Output, integer ( kind = 4 ) IB, the half bandwidth of the matrix.
!
  implicit none
...
!    is the index of the vertical velocity variable associated with the node.
!
!    Input, integer ( kind = 4 ) NODE_P_CONDITION(NODE_NUM),
!    indicates the condition used to determine pressure at a node.
...
!    3, a Neumann condition is used.
!
!    Input, integer ( kind = 4 ) NODE_U_CONDITION(NODE_NUM),
!    indicates the condition used to determine horizontal velocity at a node.
!    0, there is no condition at this node.
!    1, a finite element equation is used;
!    2, a Dirichlet condition is used.
!    3, a Neumann condition is used.
!
!    Input, integer ( kind = 4 ) NODE_V_CONDITION(NODE_NUM),
!    indicates the condition used to determine vertical velocity at a node.
!    0, there is no condition at this node.
!    1, a finite element equation is used;
!    2, a Dirichlet condition is used.
...
!    banded format; on output, the matrix has been adjusted for
...
subroutine file_name_specification ( node_file_name, triangle_file_name )
...
!
!    Output, character ( len = * ) TRIANGLE_FILE_NAME, the name
...
!
  if ( 2 <= arg_num ) then
    ...
    iarg = 2
    call getarg ( iarg, triangle_file_name )
...
!
  ...
  integer ( kind = 4 ) iu
  integer ( kind = 4 ) iv
  integer ( kind = 4 ), parameter :: NEUMANN = 3
  integer ( kind = 4 ) node
  integer ( kind = 4 ) node_p_condition(node_num)
  integer ( kind = 4 ) node_p_variable(node_num)
  integer ( kind = 4 ) node_u_condition(node_num)
...
!
!  This is where U gets involved!
!
!     f(iu) = f(iu) + line integral of U

    end if

    if ( node_v_condition(node) == NEUMANN ) then
!
!  This is where V gets involved!
!
...
!
!    Input, integer ( kind = 4 ) NODE_TYPE(NODE_NUM), determines if the node is
!    a vertex or midside node.
!    1, the node is a vertex (P, U, V variables are associated with it).
!    2, the node is a midside node (only U and V variables are associated.)
!
  ...
  integer ( kind = 4 ) node
  integer ( kind = 4 ), dimension(node_num) :: node_type
  real ( kind = 8 ), dimension(2,node_num) :: node_xy
...
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 4 ) NODE3_LABEL(NODE_NUM), contains the renumbered
!    label of order3 nodes, and -1 for nodes that are not order3 nodes.
!
  implicit none
  ...
  integer ( kind = 4 ) file_status
  integer ( kind = 4 ) file_unit
  integer ( kind = 4 ) node3_label(node_num)
  ...
  do triangle = 1, element_num
    ...
    write ( file_unit, '(2x,i8,2x,i8,2x,i8)' ) &
      node3_label( element_node(1:3,triangle) )
...
subroutine triangulation_order6_bound_node ( node_num, element_order, &
  element_num, element_node, node_boundary )

!*****************************************************************************80
!
!! TRIANGULATION_ORDER6_BOUND_NODE indicates which nodes are on the boundary.
!
!  Discussion:
!
...
!
  ...
  integer ( kind = 4 ) e1(6*element_num)
  integer ( kind = 4 ) e2(6*element_num)
  integer ( kind = 4 ) edge(2,6*element_num)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) m
  integer ( kind = 4 ) n
...
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of triangles.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), lists, for each
...
!
!    Input, integer ( kind = 4 ) NODE_U_VARIABLE(NODE_NUM), 
!    NODE_V_VARIABLE(NODE_NUM), the indices of the horizontal and vertical 
!    velocity variables associated with the node, or -1 if there is none.
!
!    Input, integer ( kind = 4 ) VARIABLE_NUM, the number of variables.
!
!    Input, real ( kind = 8 ) NODE_C(VARIABLE_NUM), the finite element
