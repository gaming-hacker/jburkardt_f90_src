...
!
!    Input, integer ( kind = 4 ) NODE_TYPE(NODE_NUM), determines if the node is a 
...
!
  ...
  logical, parameter :: neumann_use = .false.
  integer ( kind = 4 ) node
...
!
    else if ( ( .not. neumann_use ) .and. &
      x == 1.0D+00 .and. ( 0.75D+00 <= y .and. y <= 1.00D+00 ) ) then
...
!
    if ( neumann_use .and. &
      x == 1.0D+00 .and. ( 0.75D+00 <= y .and. y <= 1.0D+00 ) ) then
