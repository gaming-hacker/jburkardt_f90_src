...
c  Discussion:
c
c    The C83 storage format is used for a real tridiagonal matrix.
...
c
      ...
      subroutine r83s_mxv ( n, a, x, b )

c*********************************************************************72
c
cc R83S_MXV multiplies an R83S matrix times a vector.
c
c  Discussion:
c
...
c
      ...
      subroutine r83s_print ( n, a, title )

c*********************************************************************72
c
cc R83S_PRINT prints an R83S matrix.
c
c  Discussion:
c
c    The R83S storage format is used for a tridiagonal scalar matrix.
...
c    N must be positive.
c
c    Input, double precision A(3), the R83S matrix.
c
c    Input, character * ( * ) TITLE, a title.
c
      implicit none
      ...
      call r83s_print_some ( n, a, 1, 1, n, n, title )

      return
      end
      subroutine r83s_print_some ( n, a, ilo, jlo, ihi, jhi, title )

c*********************************************************************72
c
cc R83S_PRINT_SOME prints some of an R83S matrix.
c
c  Discussion:
c
c    The R83S storage format is used for a tridiagonal scalar matrix.
...
c
          do j2 = 1, inc
            ...
            if ( 1 .lt. i - j .or. 1 .lt. j - i ) then
              ...
              write ( ctemp(j2), '(g14.6)' ) a(1)
            else if ( j .eq. i ) then
              write ( ctemp(j2), '(g14.6)' ) a(2)
            else if ( j .eq. i + 1 ) then
              write ( ctemp(j2), '(g14.6)' ) a(3)
            end if
      ...
      subroutine r83s_random ( n, seed, a )

c*********************************************************************72
c
cc R83S_RANDOM randomizes an R83S matrix.
c
c  Discussion:
c
c    The R83S storage format is used for a tridiagonal scalar matrix.
...
c    number generator.
c
c    Output, double precision A(3), the R83S matrix.
c
      implicit none
...
c
      ...
      subroutine r8ge_to_r8ri ( n, a, nz, ija, sa )

c*********************************************************************72
c
cc R8GE_TO_R8RI converts a R8GE matrix to R8RI form.
c
c  Discussion:
c
c    A R8GE matrix is in general storage.
c
c    An R8RI matrix is in row indexed sparse storage form.
c
c    The size of the arrays IJA and SA can be determined by calling
c    R8GE_TO_R8RI_SIZE.
c
c  Licensing:
c
...
c    Input, integer N, the order of the matrix.
c
c    Input, double precision A(N,N), the matrix stored in GE 
c    or "general" format.
c
c    Input, integer NZ, the size required for the RI
c    or "row indexed" sparse storage.
c
c    Output, integer IJA(NZ), the index vector.
c
c    Output, double precision SA(NZ), the value vector.
c
      implicit none
...
c*********************************************************************72
c
cc R8GE_TO_R8RI_SIZE determines the size of an R8RI matrix.
c
c  Discussion:
c
c    N spaces are always used for the diagonal entries, plus a dummy.
c    The remaining spaces store off-diagonal nonzeros.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    or "general" format.
c
c    Output, integer NZ, the size required for the RI
c    or "row indexed" sparse storage.
c
      implicit none
...
c
      ...
      subroutine r8ri_to_r8ge ( nz, ija, sa, n, a )

c*********************************************************************72
c
cc R8RI_TO_R8GE converts an R8RI matrix to R8GE form.
c
c  Discussion:
c
c    An R8RI matrix is in row indexed sparse storage form.
...
c    or "row indexed" sparse storage.
c
c    Input, integer IJA(NZ), the index vector.
c
c    Input, double precision SA(NZ), the value vector.
c
c    Input, integer N, the order of the matrix.
c
c    Output, double precision A(N,N), the matrix stored in GE 
c    or "general" format.
c
      implicit none
