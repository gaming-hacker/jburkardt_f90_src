...
!
  ...
  do i2 = 0, n2 - 1
    do j2 = 0, n2 - 1
      theta2 = 2.0E+00 * r4_pi * real ( i2 * j2, kind = 4 ) / real ( n2, kind = 4 )
...
!
  ...
  do j = 1, n
    do i = 1, m
      ...
      theta = 2.0D+00 * r4_pi * ( real ( seed, kind = 4 ) * 4.656612875E-10 )
...
!
  ...
  do i2 = 0, n2 - 1
    do j2 = 0, n2 - 1
      theta2 = 2.0D+00 * r8_pi * real ( i2 * j2, kind = 8 ) / real ( n2, kind = 8 )
      cs2 = cmplx ( cos ( theta2 ), - sin ( theta2 ), kind = 8 )
      do i1 = 0, n1 - 1
        do j1 = 0, n1 - 1
          theta1 = 2.0D+00 * r8_pi * real ( i1 * j1, kind = 8 ) &
            / real ( n1, kind = 8 )
...
!
  ...
  do i2 = 0, n2 - 1
    do j2 = 0, n2 - 1
      theta2 = - 2.0D+00 * r8_pi * real ( i2 * j2, kind = 8 ) &
        / real ( n2, kind = 8 )
      cs2 = cmplx ( cos ( theta2 ), - sin ( theta2 ), kind = 8 )
      do i1 = 0, n1 - 1
        do j1 = 0, n1 - 1
...
!
  ...
  do i = 0, n - 1
    do j = 0, n - 1
      theta = - 2.0D+00 * r8_pi * real ( i * j, kind = 8 ) / real ( n, kind = 8 )
...
!
  ...
  do i = 1, n
    do k = 1, n / 2
      theta = real ( k * ( i - 1 ) * 2, kind = 8 ) * r8_pi &
        / real ( n, kind = 8 )
      r(i) = r(i) + a(k) * cos ( theta ) + b(k) * sin ( theta )
    end do
  end do
...
!
  ...
  do i = 1, n / 2
    ...
    do j = 1, n
      theta = real ( 2 * i * ( j - 1 ), kind = 8 ) * r8_pi &
        / real ( n, kind = 8 )
      a(i) = a(i) + r(j) * cos ( theta )
      b(i) = b(i) + r(j) * sin ( theta )
    end do
...
!
  ...
  do i = 0, n - 1
    do j = 0, n - 1
      theta = 2.0D+00 * r8_pi * real ( mod ( i * j, n ), kind = 8 ) &
        / real ( n, kind = 8 )
      b(i) = b(i) + a(j) * ( cos ( theta ) + sin ( theta ) )
    end do
  end do
...
!
  ...
  do i = 0, n - 1
    do j = 1, n - 1
      ...
      theta = 0.5D+00 * r8_pi * real ( j * ( 2 * i + 1 ), kind = 8 ) &
        / real ( n, kind = 8 )
...
!
  ...
  do i = 0, n - 1
    do j = 0, n - 1
      theta = 0.5D+00 * r8_pi * real ( i * ( 2 * j + 1 ), kind = 8 ) &
        / real ( n, kind = 8 )
      y(i) = y(i) + x(j) * cos ( theta  )
    end do
  end do
...
!
  ...
  do i = 1, n
    theta(i) = r8_pi * real ( i, kind = 8 ) / real ( n + 1, kind = 8 )
  end do
...
!    Input, integer ( kind = 4 ) N, the number of data values.
!
!    Input, real ( kind = 8 ) S((N+1)/2), D((N+1)/2), the transformed data.
!
!    Output, real ( kind = 8 ) X(N), the original data sequence.
!
  implicit none
  ...
  real ( kind = 8 ) d((n+1)/2)
  real ( kind = 8 ) d1((n+1)/2)
  real ( kind = 8 ) d2((n+1)/2)
  real ( kind = 8 ) s((n+1)/2)
  real ( kind = 8 ) s1((n+1)/2)
  real ( kind = 8 ) s2((n+1)/2)
  real ( kind = 8 ) s3((n+1)/2)
  integer ( kind = 4 ) n2
  integer ( kind = 4 ) nh
  integer ( kind = 4 ) np1h
  real ( kind = 8 ) x(n)
  real ( kind = 8 ) y(2*((n+1)/2))
...
!    Input, real ( kind = 8 ) X(N), the data sequence.
!
!    Output, real ( kind = 8 ) S((N+1)/2), D((N+1)/2), the transformed data.
!
  implicit none
