...
!
  ...
  fx = ( r8_pi * ( x - m ) / 180.0D+00 ) - e * sin ( r8_pi * x / 180.0D+00 )
...
!
  ...
  fx1 = ( r8_pi / 180.0D+00 ) &
    - e * r8_pi * cos ( r8_pi * x / 180.0D+00  ) / 180.0D+00
...
!
  ...
  fx2 = e * r8_pi * r8_pi * sin ( r8_pi * x / 180.0D+00  ) / 180.0D+00**2
...
!
  ...
  arg = - ( 10.0D+00 - 30.0D+00 * x )**2
  fx1 = - 100.0D+00 * sin ( 100.0D+00 * x ) &
    + 240.0D+00 * exp ( arg ) / sqrt ( pi )

  return
end
subroutine p19_fx2 ( x, fx2 )
...
!
  ...
  arg = - ( 10.0D+00 - 30.0D+00 * x )**2
  fx2 = - 10000.0D+00 * cos ( 100.0D+00 * x ) &
    + 14400.0D+00 * exp ( arg ) * ( 10.0D+00 - 30.0D+00 * x ) / sqrt ( pi )

  return
end
subroutine p19_range ( range )
...
!
  ...
  if ( i == 1 ) then
    x = 0.33186603357456253747D+00
  else
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'P19_ROOT - Fatal error!'
    write ( *, '(a,i4)' ) '  Illegal root index = ', i
    stop
  end if
...
subroutine p19_root_num ( root_num )
...
subroutine p19_start_num ( start_num )

!*****************************************************************************80
!
!! P19_START_NUM returns the number of starting points for P19.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
