#!/bin/bash
#
gfortran -c tetrahedron_arbq_rule_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling tetrahedron_arbq_rule_prb.f90"
  exit
fi
#
gfortran tetrahedron_arbq_rule_prb.o -L$HOME/lib/$ARCH -ltetrahedron_arbq_rule
if [ $? -ne 0 ]; then
