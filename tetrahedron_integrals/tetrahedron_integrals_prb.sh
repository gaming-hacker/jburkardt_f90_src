#!/bin/bash
#
gfortran -c tetrahedron_integrals_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling tetrahedron_integrals_prb.f90"
  exit
fi
#
gfortran tetrahedron_integrals_prb.o -L$HOME/lib/$ARCH -ltetrahedron_integrals
if [ $? -ne 0 ]; then
