#!/bin/bash
#
gfortran -c triangle_felippa_rule_prb.f90
if [ $? -ne 0 ]; then
  echo "Errors compiling triangle_felippa_rule_prb.f90"
  exit
fi
#
gfortran triangle_felippa_rule_prb.o -L$HOME/lib/$ARCH -ltriangle_felippa_rule
if [ $? -ne 0 ]; then
