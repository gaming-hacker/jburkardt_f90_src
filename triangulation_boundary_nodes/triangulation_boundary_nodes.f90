...
!
  ...
  character ( len = 255 ) boundarynode_filename
  ...
  integer ( kind = 4 ) node_boundary_num
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGULATION_BOUNDARY_NODES'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Identify triangulation nodes that lie on the boundary.'
  write ( *, '(a)' ) ' '
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '* Determine which nodes are on the boundary;'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '* Write a file which is 1 for each boundary node.'
!
...
!
  ...
  boundarynode_filename = trim ( prefix ) // '_boundary_nodes.txt'
!
!  Read the node data.
!
...
!
  call i4mat_header_read ( element_filename, &
  ...
  call i4mat_transpose_print_some ( triangle_order, triangle_num, &
    triangle_node, 1, 1, triangle_order, 5, '  Portion of triangle file:' )
!
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Number of boundary nodes is ', node_boundary_num

  if ( node_boundary_num < 100 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Boundary nodes:'
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '     New     Old'
    write ( *, '(a)' ) '   Index   Index         X and Y Coordinates'
    write ( *, '(a)' ) ' '
    node2 = 0
    do node = 1, node_num
      if ( node_boundary(node) ) then
...
!
  call i4mat_write ( boundarynode_filename, 1, node_num, node_boundary )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Wrote the node boundary value file "' &
    // trim ( boundarynode_filename ) //'".'
!
!  Free memory.
!
...
subroutine i4mat_write ( node_boundary_filename, m, n, table )
...
!
  ...
  integer ( kind = 4 ) j
  character ( len = * )  node_boundary_filename
...
!
  ...
  open ( unit = output_unit, file = node_boundary_filename, &
  ...
  if ( output_status /= 0 ) then
    ...
    write ( *, '(a,i8)' ) '  Could not open the output file "' // &
      trim ( node_boundary_filename ) // '" on unit ', output_unit
    output_unit = -1
    stop
  end if
!
...
subroutine r8mat_write ( node_boundary_filename, m, n, table )
...
!
!    Input, integer ( kind = 4 ) ISGN, results of comparing elements I and J.
